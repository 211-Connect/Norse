...
import { invalidateApiCache } from '@/payload/collections/ResourceDirectories/hooks/invalidateApiCache';
import { pushFacetsToCache } from '@/payload/collections/ResourceDirectories/hooks/pushFacetsToCache';
import { revalidateCache } from '@/payload/collections/ResourceDirectories/hooks/revalidateCache';
...
import { SKIP_SIDE_EFFECTS_CONTEXT_KEY } from '../utilities/hookContext';
...
/**
 * Runs, once per job, the side effects that ResourceDirectories.afterChange
 * would otherwise run once per translated locale (facets/hybrid-search cache
 * pushes, API cache invalidation). Keycloak branding is intentionally skipped
 * here: translated locales never change realm attributes (theme/logo/title
 * come from the `en` doc), so re-syncing per job would be wasted HTTP calls.
 */
async function runPostTranslationSideEffects(
  req: PayloadRequest,
  tenantId: string,
  englishResourceDirectory: ResourceDirectory,
): Promise<void> {
  try {
    await revalidateCache({ doc: englishResourceDirectory });
    await pushFacetsToCache(englishResourceDirectory, req, 'en');
    await pushHybridSearchConfigToCache(tenantId, req, {
      resourceDirectory: englishResourceDirectory,
    });
    await invalidateApiCache({ doc: englishResourceDirectory });
  } catch (error) {
    log.error(
      { err: error, tenantId },
      'Error running post-translation cache side effects',
    );
  }
}
...
export const translate: TaskConfig<'translate'> = {
  ...
  handler: async ({ input, job, req }) => {
    ...
    // Multiple replicas poll the same 'translation' queue; only one may execute this job.
        ...
        try {
          ...
          for (const targetLocale of locales) {
            ...
            SEARCH_TEXT_FIELDS.forEach((field) => {
              ...
              const sourceValue =
                englishResourceDirectory.search?.texts?.[field];
            ...
            // Badges
            englishResourceDirectory.badges?.list?.forEach(
              (sourceBadge, index) => {
                ...
                (['badgeLabel', 'tooltip'] as const).forEach((field) => {
                  ...
                  if (
                    shouldTranslate(sourceValue, targetValue, sourceBadge.id)
                  ) {
                    fieldsToTranslate.push({
            ...
            if (translationsByPath['brand.printableDocuments.ctaText']) {
              updateData.brand = {
                ...
                printableDocuments: {
                  ...
                  ctaText:
                    translationsByPath['brand.printableDocuments.ctaText'],
                },
              };
            ...
            if (translationsByPath['topics.backText']) {
              updateData.topics!.backText =
                translationsByPath['topics.backText'];
            ...
            if (
              translationsByPath['search.texts.suggestionHeaders.categories']
            ) {
              updateData.search = {
                ...targetDoc.search,
                texts: {
              ...
              if (translationsByPath[path]) {
                ...
                updateData.search!.texts = {
                  ...
                  suggestionHeaders: {
                    ...updateData.search!.texts?.suggestionHeaders,
                    [field]:
                      englishResourceDirectory.search?.texts
                        ?.suggestionHeaders?.[field],
                  },
                };
              }
            });
            ...
            if (englishResourceDirectory.search?.facets) {
              ...
              updateData.search!.facets =
                englishResourceDirectory.search.facets.map(
                  (sourceFacet, index) => {
            ...
            if (translationsByPath['common.customDataProvidersHeading']) {
              if (!updateData.common)
                updateData.common = { ...targetDoc.common };
                ...
                englishResourceDirectory.common.dataProviders.map(
                  (sourceProvider, index) => {
                    const targetProvider =
                      targetDoc.common?.dataProviders?.[index];
                    const newProvider = {
                      ...sourceProvider,
                      ...(targetProvider || {}),
                    };
                  ...
                  englishResourceDirectory.highlights.items.map(
                    (sourceHighlight, index) => {
                      const targetHighlight =
                        targetDoc.highlights?.items?.[index];
                      const newHighlight = {
                        ...sourceHighlight,
                        ...(targetHighlight || {}),
                      };
            ...
            (['leftColumn', 'rightColumn'] as const).forEach((columnKey) => {
              const sourceColumn =
                englishResourceDirectory.resource?.[columnKey];
              ...
              const newColumn = sourceColumn.map((sourceGroup, groupIndex) => {
                ...
                newGroup.items = sourceGroup.items?.map(
                  (sourceItem, itemIndex) => {
                    ...
                    if (sourceItem.customAttribute) {
                      ...
                      CUSTOM_ATTRIBUTE_TEXT_FIELDS.forEach((field) => {
                        ...
                        if (translationsByPath[path]) {
                          ...
                          isEmpty(targetItem?.customAttribute?.[field])
                        ) {
            ...
            if (englishResourceDirectory.search?.cardLayout) {
              updateData.search!.cardLayout =
                englishResourceDirectory.search.cardLayout.map(
                  (sourceItem, itemIndex) => {
                    const targetItem =
                      targetDoc.search?.cardLayout?.[itemIndex];
                    const newItem = {
                      ...sourceItem,
                      ...(targetItem || {}),
                      id: sourceItem.id,
            ...
            await payload.update({
              ...
              data: updateData,
              req,
              // Each locale write would otherwise re-run the full afterChange
              // chain (facets/hybrid-search/Keycloak/API-cache); run those once
              // below instead of 10x per job.
              context: { [SKIP_SIDE_EFFECTS_CONTEXT_KEY]: true },
            });

            totalTranslatedCounts[targetLocale] = fieldsToTranslate.length;
          }

          if (Object.keys(totalTranslatedCounts).length > 0) {
            await runPostTranslationSideEffects(
              req,
              tenantId,
              englishResourceDirectory,
            );
    ...
    if (!lockResult.acquired) {
      log.warn(
        ...
        'Translate job already running on another instance; skipping duplicate execution',
      );
      return { output: { success: true, translated: 0 } };
    }
