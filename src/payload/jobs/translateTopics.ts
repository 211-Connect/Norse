  ...
  handler: async ({ input, job, req }) => {
    ...
    const lockResult = await withJobLock(
      ...
      async () => {
        try {
          ...
          log.debug(
            { tenantId },
            'Resource directory found; collecting fields',
          );
          ...
          for (const localeString of locales) {
            ...
            if (
              ...
              resourceDirectory.topics.list.forEach(
                (topic, topicIndex: number) => {
                  if (topic?.name) {
                    const localizedTopic =
                      localizedDoc.topics?.list?.[topicIndex];
          ...
          for (let i = 0; i < fieldsToTranslate.length; i += batchSize) {
            ...
            log.debug(
              { tenantId, processedCount, totalFields },
              'Batch completed',
            );
          }
    ...
    if (!lockResult.acquired) {
      log.warn(
        ...
        'translateTopics job already running on another instance; skipping duplicate execution',
      );
