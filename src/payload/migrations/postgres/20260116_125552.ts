...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    ADD VALUE 'translate' BEFORE 'warmCache';
  ...
  const createMappingsAndMigrate = sql`
    ...
    SELECT
      ...
      s_en."id" AS en_id
    FROM
      "rds_suggestions" s_all
      INNER JOIN "rds_suggestions" s_en ON s_all."_parent_id" = s_en."_parent_id"
      AND s_all."_order" = s_en."_order"
      AND s_en."_locale" = 'en';

    INSERT INTO
      "rds_suggestions_locales" ("value", "_locale", "_parent_id")
    SELECT
      locale_value,
      locale,
      en_id
    FROM
      suggestion_en_mapping;
    ...
    SELECT
      ...
      s_en."id" AS en_id
    FROM
      "_rds_v_version_suggestions" s_all
      INNER JOIN "_rds_v_version_suggestions" s_en ON s_all."_parent_id" = s_en."_parent_id"
      AND s_all."_order" = s_en."_order"
      AND s_en."_locale" = 'en';

    INSERT INTO
      "_rds_v_version_suggestions_locales" ("value", "_locale", "_parent_id")
    SELECT
      locale_value,
      locale,
      en_id
    FROM
      version_suggestion_en_mapping;
  `;

  await db.execute(createMappingsAndMigrate);

  await db.execute(sql`
    DELETE FROM "rds_suggestions"
    WHERE
      "_locale" != 'en';

    DELETE FROM "_rds_v_version_suggestions"
    WHERE
      "_locale" != 'en';
  `);
  ...
  await db.execute(sql`
    ...
    ADD CONSTRAINT "rds_suggestions_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."rds_suggestions" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;

    ALTER TABLE "_rds_v_version_suggestions_locales"
    ADD CONSTRAINT "_rds_v_version_suggestions_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."_rds_v_version_suggestions" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;
...
export async function down({
  ...
  await db.execute(sql`
    ...
    ALTER TABLE "payload_jobs_log"
    ALTER COLUMN "task_slug"
    SET DATA TYPE "public"."enum_payload_jobs_log_task_slug" USING "task_slug"::"public"."enum_payload_jobs_log_task_slug";
    ...
    ALTER TABLE "payload_jobs"
    ALTER COLUMN "task_slug"
    SET DATA TYPE "public"."enum_payload_jobs_task_slug" USING "task_slug"::"public"."enum_payload_jobs_task_slug";
