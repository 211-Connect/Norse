...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  const createMappingsAndMigrate = sql`
    ...
    SELECT
      ...
      t_en."id" AS en_id
    FROM
      "rds_topics_list" t_all
      INNER JOIN "rds_topics_list" t_en ON t_all."_parent_id" = t_en."_parent_id"
      AND t_all."_order" = t_en."_order"
      AND t_en."_locale" = 'en';

    -- Migrate topics using the English ID as _parent_id
    INSERT INTO
      "rds_topics_list_locales" ("name", "_locale", "_parent_id")
    SELECT
      locale_name,
      locale,
      en_id
    ...
    SELECT
      ...
      st_en."id" AS en_id
    FROM
      "rds_topics_list_subtopics" st_all
      -- Join to find the English topic for the non-English parent
      INNER JOIN "rds_topics_list" t_parent ON st_all."_parent_id" = t_parent."id"
      INNER JOIN "rds_topics_list" t_parent_en ON t_parent."_parent_id" = t_parent_en."_parent_id"
      ...
      INNER JOIN "rds_topics_list_subtopics" st_en ON st_en."_parent_id" = t_parent_en."id"
      AND st_all."_order" = st_en."_order"
      AND st_en."_locale" = 'en';

    INSERT INTO
      "rds_topics_list_subtopics_locales" ("name", "_locale", "_parent_id")
    SELECT
      locale_name,
      locale,
      en_id
    ...
    SELECT
      ...
      t_en."id" AS en_id
    FROM
      "_rds_v_version_topics_list" t_all
      INNER JOIN "_rds_v_version_topics_list" t_en ON t_all."_parent_id" = t_en."_parent_id"
      AND t_all."_order" = t_en."_order"
      AND t_en."_locale" = 'en';

    INSERT INTO
      "_rds_v_version_topics_list_locales" ("name", "_locale", "_parent_id")
    SELECT
      locale_name,
      locale,
      en_id
    FROM
      version_topic_en_mapping;
    ...
    SELECT
      ...
      st_en."id" AS en_id
    FROM
      "_rds_v_version_topics_list_subtopics" st_all
      -- Join to find the English topic for the non-English parent
      INNER JOIN "_rds_v_version_topics_list" t_parent ON st_all."_parent_id" = t_parent."id"
      INNER JOIN "_rds_v_version_topics_list" t_parent_en ON t_parent."_parent_id" = t_parent_en."_parent_id"
      AND t_parent."_order" = t_parent_en."_order"
      AND t_parent_en."_locale" = 'en'
      -- Now match with English subtopic by same English parent + order
      INNER JOIN "_rds_v_version_topics_list_subtopics" st_en ON st_en."_parent_id" = t_parent_en."id"
      AND st_all."_order" = st_en."_order"
      AND st_en."_locale" = 'en';

    INSERT INTO
      "_rds_v_version_topics_list_subtopics_locales" ("name", "_locale", "_parent_id")
    SELECT
      locale_name,
      locale,
      en_id
    FROM
      version_subtopic_en_mapping;
  `;
  ...
  await db.execute(sql`
    ...
    ALTER TABLE "rds_topics_list_subtopics_locales"
    ADD CONSTRAINT "rds_topics_list_subtopics_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."rds_topics_list_subtopics" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;

    ALTER TABLE "rds_topics_list_locales"
    ADD CONSTRAINT "rds_topics_list_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."rds_topics_list" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;

    ALTER TABLE "_rds_v_version_topics_list_subtopics_locales"
    ADD CONSTRAINT "_rds_v_version_topics_list_subtopics_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."_rds_v_version_topics_list_subtopics" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;

    ALTER TABLE "_rds_v_version_topics_list_locales"
    ADD CONSTRAINT "_rds_v_version_topics_list_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."_rds_v_version_topics_list" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;
