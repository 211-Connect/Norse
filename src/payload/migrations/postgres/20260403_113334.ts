...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  FROM "rds_common_alert" alert_all
  INNER JOIN "rds_common_alert" alert_en 
    ON alert_all."_parent_id" = alert_en."_parent_id" 
    AND alert_all."_order" = alert_en."_order"
    AND alert_en."_locale" = 'en';
  
  INSERT INTO "rds_common_alert_locales" ("text", "button_text", "_locale", "_parent_id")
  SELECT locale_text, locale_button_text, locale, en_id
  FROM alert_en_mapping;
  ...
  FROM "rds_new_layout_callouts_options" callout_all
  INNER JOIN "rds_new_layout_callouts_options" callout_en 
    ON callout_all."_parent_id" = callout_en."_parent_id" 
    AND callout_all."_order" = callout_en."_order"
    AND callout_en."_locale" = 'en';
  
  INSERT INTO "rds_new_layout_callouts_options_locales" ("description", "title", "_locale", "_parent_id")
  SELECT locale_description, locale_title, locale, en_id
  FROM callouts_en_mapping;
  ...
  SELECT 
    ...
    alert_all."button_text" as locale_button_text,
    alert_en."id" as en_id
  FROM "_rds_v_version_common_alert" alert_all
  INNER JOIN "_rds_v_version_common_alert" alert_en 
    ON alert_all."_parent_id" = alert_en."_parent_id" 
    AND alert_all."_order" = alert_en."_order"
    AND alert_en."_locale" = 'en';
  
  INSERT INTO "_rds_v_version_common_alert_locales" ("text", "button_text", "_locale", "_parent_id")
  SELECT locale_text, locale_button_text, locale, en_id
  FROM version_alert_en_mapping;
  ...
  SELECT 
    ...
    callout_all."title" as locale_title,
    callout_en."id" as en_id
  FROM "_rds_v_version_new_layout_callouts_options" callout_all
  INNER JOIN "_rds_v_version_new_layout_callouts_options" callout_en 
    ON callout_all."_parent_id" = callout_en."_parent_id" 
    AND callout_all."_order" = callout_en."_order"
    AND callout_en."_locale" = 'en';
  
  INSERT INTO "_rds_v_version_new_layout_callouts_options_locales" ("description", "title", "_locale", "_parent_id")
  SELECT locale_description, locale_title, locale, en_id
  FROM version_callouts_en_mapping;
  `;
  ...
  await db.execute(sql`
  DELETE FROM "rds_common_alert" WHERE "_locale" != 'en';
  DELETE FROM "rds_new_layout_callouts_options" WHERE "_locale" != 'en';
  DELETE FROM "_rds_v_version_common_alert" WHERE "_locale" != 'en';
  DELETE FROM "_rds_v_version_new_layout_callouts_options" WHERE "_locale" != 'en';
  `);
