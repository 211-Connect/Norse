...
export const generateMetadata = async ({
  ...
  if (useGeospatialSearch) {
    ...
    const placeMetadata = await geocodeLocationCached(searchParamsResult.location, locale);
    ...
    if (placeMetadata) {
      const searchStore = {
        ...
        searchCoordinates: searchParamsResult.coords?.split(',').map(Number) || placeMetadata.coordinates,
      ...
      try {
        ...
        console.error('Geospatial search failed, falling back to legacy:', error);
        // Fallback below
      }
    }
  }
  ...
  if (useGeospatialSearch) {
    ...
    if (placeMetadata) {
      const searchStore = {
        ...
        searchCoordinates: coords?.split(',').map(Number) || placeMetadata.coordinates,
        searchDistance: searchParamsResult.distance || '0',
        searchPlaceType: placeMetadata.place_type,
        searchBbox: placeMetadata.bbox,
      };
      ...
      try {
        ...
        ({ results, noResults, page, totalResults, filters } = await findResourcesV2(
          searchStore,
          locale,
          parseInt((searchParamsResult?.page as string) ?? '1'),
          limit,
