...
export function setLocationCookies(address: string, result: GeocodeResult) {
  ...
  if (result.country) setCookie(USER_PREF_COUNTRY, result.country, COOKIE_OPTIONS);
  if (result.district) setCookie(USER_PREF_DISTRICT, result.district, COOKIE_OPTIONS);
  if (result.place) setCookie(USER_PREF_PLACE, result.place, COOKIE_OPTIONS);
  if (result.postcode) setCookie(USER_PREF_POSTCODE, result.postcode, COOKIE_OPTIONS);
