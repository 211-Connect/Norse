...
const getFavoriteList = cache(
  async (
    ...
    try {
      const { data } = await createAxiosWithAuth({ sessionId, tenantId }).get(
...
const getFavoriteLists = cache(
  async (sessionId?: string, tenantId?: string) => {
    try {
      const { data } = await createAxiosWithAuth({ sessionId, tenantId }).get(
...
const searchFavoriteLists = cache(
  async (searchText: string, sessionId?: string, tenantId?: string) => {
    try {
      const { data } = await createAxiosWithAuth({ sessionId, tenantId }).get(
...
const updateFavoriteList = async (
  ...
  try {
    const { data } = await createAxiosWithAuth({ sessionId, tenantId }).put(
      `${API_URL}/${listEndpoint}/${id}`,
      {
        name,
        description,
...
const deleteFavoriteList = async (
  ...
  try {
    const { data } = await createAxiosWithAuth({ sessionId, tenantId }).delete(
      `${API_URL}/${listEndpoint}/${id}`,
      {
        headers: {
          'x-api-version': '1',
...
const createFavoriteList = async (
  ...
  try {
    const { data } = await createAxiosWithAuth({ sessionId, tenantId }).post(
      `${API_URL}/${listEndpoint}`,
      {
        name,
        description,
