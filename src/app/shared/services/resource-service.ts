...
export const getResource = cache((id: string, locale: string): Promise<any> => {
  const url = `${API_URL}/resource/${id}`;
  return fetchAndTransformResource(url, { locale });
});

export const getResourceByOriginalId = cache(
  (originalId: string, locale: string): Promise<any> => {
