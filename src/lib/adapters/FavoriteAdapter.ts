...
export class FavoriteAdapter extends BaseAdapter {
  public async getFavoriteList(id: string | string[]) {
    const { data } = await this.axios.get(`/api/favorite-list/${id}`);
  ...
  public async updateFavoriteList({
    ...
    const { data } = await this.axios.put(`/api/favorite-list/${id}`, {
      name,
      description,
      public: privacy,
    });
  ...
  public async deleteFavoriteList(id: string) {
    const { data } = await this.axios.delete(`/api/favorite-list/${id}`);
  ...
  public async searchFavoriteLists({
    ...
    const { data } = await this.axios.get(`/api/favorite-list/search?${query}`);
  ...
  public async addToFavoriteList({
    ...
    const { data } = await this.axios.post('/api/favorite', {
